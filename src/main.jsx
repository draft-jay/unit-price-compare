...
function App() {
  ...
  return <div className="app-shell">
    <main>
      {tab === 'compare' && <Compare items={items} form={form} update={update} addItem={addItem} clearForm={() => { setForm(blankForm); setError('') }} removeItem={removeItem} clearItems={() => setItems([])} restore={restore} saveItem={saveItem} error={error} />}
...
function Compare({ items, form, update, addItem, clearForm, removeItem, clearItems, restore, saveItem, error }) {
  ...
  return <>
    <form className="entry-card" onSubmit={addItem}>
      ...
      <div className="form-row"><label>规格<input autoFocus inputMode="decimal" value={form.amount} onChange={(e) => update('amount', e.target.value)} /></label><label>价格<input inputMode="decimal" value={form.price} onChange={(e) => update('price', e.target.value)} /></label><label>单位<select value={form.unit} onChange={(e) => update('unit', e.target.value)}><option value="g">g</option><option value="ml">ml</option></select></label></div>
      {error && <p className="error">{error}</p>}<div className="form-actions"><button className="primary" type="submit">添加到比价 <span>＋</span></button><button className="clear-form" type="button" onClick={clearForm} aria-label="清空输入框" title="清空输入框">⌫</button></div>
    </form>
    <section className="results"><div className="section-heading"><h3>当前比较 <span>{items.length}</span></h3>{items.length > 0 && <button className="text-button" onClick={clearItems}>清空</button>}</div>
      {items.length === 0 ? <div className="empty"><span>↗</span><p>还没有商品</p><small>先添加两个商品，看看谁更划算</small></div> : <><ResultGroup title="重量商品" unit="g" data={group('g')} removeItem={removeItem} restore={restore} saveItem={saveItem} /><ResultGroup title="体积商品" unit="ml" data={group('ml')} removeItem={removeItem} restore={restore} saveItem={saveItem} /></>}
    </section>
  </>
}

function ResultGroup({ title, unit, data, removeItem, restore, saveItem }) { if (!data.length) return null; return <div className="result-group"><h4>{title}<span>元/{unit === 'g' ? 'kg' : '1L'}</span></h4>{data.map((item, index) => <article className={'product-card ' + (index === 0 ? 'winner' : '')} key={item.id} onClick={() => restore(item)}>{index === 0 && <i className="best-label">最划算</i>}<div className="rank">{index + 1}</div><div className="product-info"><b>{item.name || '未命名商品'}</b><span>{item.amount}{item.unit} · {money(item.price)}元</span></div><UnitPrice item={item} /><div className="card-actions"><button className="save-button" style={{ display: 'inline' }} onClick={(event) => { event.stopPropagation(); saveItem(item) }} disabled={!item.name} title={item.name ? '保存到收藏' : '填写商品名称后才能保存'}>{item.name ? '保存' : '需名称'}</button><button onClick={(event) => { event.stopPropagation(); removeItem(item.id) }} aria-label="删除">×</button></div></article>)}</div> }
